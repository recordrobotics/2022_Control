...
public class FlywheelAutoRun extends CommandBase {
 ...
 /** Called just before this Command runs the first time*/
  ...
  public void execute() {
     ...
     */
    m_flywheel.setWheelSpeed(Constants.FLYWHEEL_SPEED);
    if (flywheelShootDurationTimer.get() > 4.5) m_flywheel.reset();
    else if (flywheelShootDurationTimer.get() > 3) m_flywheel.shoot();
    else if (flywheelShootDurationTimer.get() > 1.5) m_flywheel.reset();
    else if(flywheelShootDurationTimer.get() > 0.5) m_flywheel.shoot();
